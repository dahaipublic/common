...
func (q *DelayQueue) SendScheduleMsg(payload string, t time.Time, opts ...interface{}) error {
	...
	err := q.redisCli.Set(q.genMsgKey(idStr), payload, msgTTL).Err()
	if err != nil {
		return fmt.Errorf("store msg failed: %v", err)
	}
	// store retry count
	err = q.redisCli.HSet(q.retryCountKey, idStr, retryCount).Err()
	if err != nil {
		return fmt.Errorf("store retry count failed: %v", err)
	}
	// put to pending
	err = q.redisCli.ZAdd(q.pendingKey, &redis.Z{Score: float64(t.Unix()), Member: idStr}).Err()
	if err != nil {
...
func (q *DelayQueue) pending2Ready() error {
	...
	err := q.redisCli.Eval(pending2ReadyScript, keys, now).Err()
...
func (q *DelayQueue) ready2Unack() (string, error) {
	...
	ret, err := q.redisCli.Eval(ready2UnackScript, keys, retryTime).Result()
	if err == redis.Nil {
		return "", err
	}
	if err != nil {
...
func (q *DelayQueue) retry2Unack() (string, error) {
	...
	ret, err := q.redisCli.Eval(ready2UnackScript, keys, retryTime, q.retryKey, q.unAckKey).Result()
	if err == redis.Nil {
		return "", redis.Nil
	}
	if err != nil {
...
func (q *DelayQueue) callback(idStr string) error {
	payload, err := q.redisCli.Get(q.genMsgKey(idStr)).Result()
	if err == redis.Nil {
		return nil
	}
	if err != nil {
...
func (q *DelayQueue) ack(idStr string) error {
	err := q.redisCli.ZRem(q.unAckKey, idStr).Err()
	if err != nil {
		return fmt.Errorf("remove from unack failed: %v", err)
	}
	// msg key has ttl, ignore result of delete
	_ = q.redisCli.Del(q.genMsgKey(idStr)).Err()
	q.redisCli.HDel(q.retryCountKey, idStr)
	return nil
}

func (q *DelayQueue) nack(idStr string) error {
	// update retry time as now, unack2Retry will move it to retry immediately
	err := q.redisCli.ZAdd(q.unAckKey, &redis.Z{
		Member: idStr,
...
func (q *DelayQueue) unack2Retry() error {
	...
	now := time.Now()
	err := q.redisCli.Eval(unack2RetryScript, keys, now.Unix()).Err()
	if err != nil && err != redis.Nil {
...
func (q *DelayQueue) garbageCollect() error {
	msgIds, err := q.redisCli.SMembers(q.garbageKey).Result()
	if err != nil {
	...
	err = q.redisCli.Del(msgKeys...).Err()
	if err != nil && err != redis.Nil {
		return fmt.Errorf("del msgs failed: %v", err)
	}
	err = q.redisCli.SRem(q.garbageKey, msgIds).Err()
	if err != nil && err != redis.Nil {

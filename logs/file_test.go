...
func TestFileRotate_01(t *testing.T) {
	...
	log.Critical("critical")
	log.Emergency("emergency")
	rotateName := "test3" + fmt.Sprintf(".%s.%03d", time.Now().Format("2006-01-02.15"), 1) + ".log"
...
func TestFileRotate_02(t *testing.T) {
	fn1 := "rotate_day.log"
	fn2 := "rotate_day." + time.Now().Add(-24*time.Hour).Format("2006-01-02.15") + ".log"
	testFileRotate(t, fn1, fn2)
}

func TestFileRotate_03(t *testing.T) {
	fn1 := "rotate_day.log"
	fn := "rotate_day." + time.Now().Add(-24*time.Hour).Format("2006-01-02.15") + ".log"
	os.Create(fn)
	fn2 := "rotate_day." + time.Now().Add(-24*time.Hour).Format("2006-01-02.15") + ".001.log"
	testFileRotate(t, fn1, fn2)
	os.Remove(fn)
}
